...
from app.application.services.user.dto.auth import TokenResponse, LoginResponse
...
# /prod/user/
@router.post("/auth/google-login", response_model=LoginResponse, status_code=status.HTTP_200_OK)
def google_login(
        token: str,
...
@router.post("/auth/kakao-login", response_model=LoginResponse, status_code=status.HTTP_200_OK)
def kakao_login(
        token: str,
...
@router.post("/auth/apple-login", response_model=LoginResponse, status_code=status.HTTP_200_OK)
def apple_login(
        token: str,
