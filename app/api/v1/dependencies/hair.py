...
def get_specific_color_service(
...
) -> SpecificColorService:
    return SpecificColorService(repo=repo, color_repo=color_repo)

def get_hair_style_service(
        repo: HairStyleRepository = Depends(get_hair_style_repository),
        gender_repo: GenderRepository = Depends(get_gender_repository)
) -> HairStyleService:
    return HairStyleService(repo=repo, gender_repo=gender_repo)
...
def get_hair_variant_model_service(
    ...
    return HairVariantModelService(
        repo=repo,
