...
from app.domain.generation.services.generation_domain_service import calculate_single_generation_sec, \
    calculate_generation_eta_sec
...
class ImageGenerationRetryService:
    ...
    async def retry_expired_jobs(self):
        ...
        current_message_ttl = calculate_generation_eta_sec(
        ...
        for expired_job in expired_jobs:
            ...
            # 재요청 - mq 높은 우선순위, 생성 예상시간 mult: 1
            self.image_generation_job_repo.update(
                obj_id=expired_job.id,
                obj_in=ImageGenerationJobUpdate(
                    retry_count=expired_job.retry_count + 1,
                    expires_at=datetime.now(UTC) + timedelta(seconds=current_message_ttl)
                )
            )
            ...
            await self.rabbit_mq_service.publish(
                message=message,
                expiration_sec=current_message_ttl,
                priority=MessagePriority.URGENT
            )
            current_message_ttl += calculate_single_generation_sec(processor_count=consumer_count)
